...
export default function ProjectCard({ project }: { project: any }) {
  ...
  return (
    <motion.div 
      ...
      whileHover={showFeedback ? {} : { scale: 1.02, y: -5, boxShadow: "0 10px 30px rgba(212,175,55,0.1)" }}
      whileTap={showFeedback ? {} : { scale: 0.98 }}
      transition={{ duration: 0.3, ease: "easeOut" }}
      className={`group block w-full bg-white/[0.02] border border-white/10 hover:border-[#D4AF37]/50 rounded-2xl overflow-hidden backdrop-blur-sm relative transition-all ${showFeedback ? 'cursor-default' : 'cursor-pointer'}`}
    >
      ...
      <div className="p-5 md:p-6 relative">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button 
            ...
            className="w-full sm:w-auto text-[10px] uppercase tracking-widest px-4 py-3 sm:py-1.5 border border-[#D4AF37]/30 text-[#D4AF37] hover:bg-[#D4AF37]/10 rounded-full transition-colors opacity-100 sm:opacity-0 group-hover:opacity-100 flex items-center justify-center min-h-[44px] sm:min-h-0"
          >
            Leave Feedback
          </button>
        </div>
